...
CREATE TABLE `perekrutan_pelamar` (
  ...
  `vektor_s` bigint(20) DEFAULT NULL,
  `vektor_v` bigint(20) DEFAULT NULL,
  PRIMARY KEY (`pelamar_id`),
...
/*Table structure for table `view_nilai_test` */

DROP TABLE IF EXISTS `view_nilai_test`;

/*!50001 DROP VIEW IF EXISTS `view_nilai_test` */;
/*!50001 DROP TABLE IF EXISTS `view_nilai_test` */;

/*!50001 CREATE TABLE  `view_nilai_test`(
 `tes_id` int(11) ,
 `nama_tes` varchar(200) ,
 `bobot_tes` int(5) ,
 `jenis_tes` char(1) ,
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_data` AS (select `tu`.`tes_id` AS `tes_id`,`tu`.`nama_tes` AS `nama_tes`,`tu`.`bobot_tes` AS `bobot_tes`,`tu`.`jenis_tes` AS `jenis_tes`,`ht`.`skor_tes` AS `skor_tes`,`ht`.`pelamar_id` AS `pelamar_id`,`p`.`nama_pelamar` AS `nama_pelamar` from ((`hasil_tes_pelamar` `ht` left join `tes_ujian` `tu` on((`ht`.`tes_id` = `tu`.`tes_id`))) left join `pelamar` `p` on((`ht`.`pelamar_id` = `p`.`pelamar_id`))) order by `p`.`pelamar_id`) */;

/*View structure for view view_nilai_test */

/*!50001 DROP TABLE IF EXISTS `view_nilai_test` */;
/*!50001 DROP VIEW IF EXISTS `view_nilai_test` */;

/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_nilai_test` AS (select `tu`.`tes_id` AS `tes_id`,`tu`.`nama_tes` AS `nama_tes`,`tu`.`bobot_tes` AS `bobot_tes`,`tu`.`jenis_tes` AS `jenis_tes`,`ht`.`skor_tes` AS `skor_tes`,`ht`.`pelamar_id` AS `pelamar_id`,`p`.`nama_pelamar` AS `nama_pelamar` from ((`hasil_tes_pelamar` `ht` left join `tes_ujian` `tu` on((`ht`.`tes_id` = `tu`.`tes_id`))) left join `pelamar` `p` on((`ht`.`pelamar_id` = `p`.`pelamar_id`))) order by `tu`.`tes_id`) */;
